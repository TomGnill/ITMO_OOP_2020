...
namespace Lab5_test
{
    public class Tests
    {
        ...
        public void Setup()
        {
            ...
            Bank alfaBank = new Bank(TermForfirstBank, 0.1, 0.2, 10000, 3000);//Условия по депозиту, комиссия, процент на остаток, кредитный лимит, лимит для подозрительных аккаунтов)
            ...
            Person Andrey = new Person("Андрей", "Загудько", newAdress, firstPersonData); //Создадим одну проверенную персону.
            Person Vasya = new Person("Вася", "Дударь"); //И одну подозрительную, в некоторых тестах будем подтверждать его статус лояльности и проверять ограничения.
        ...
        public void Test1()
        {
            ...
            firstBank.CashWithdrawal(firstAndreyAccount, 1100);//Снимаем больше чем имеем, поскольку аккаунт дебитовый деньги не снимутся
        ...
        public void Test2()//Проверяем начисление процента на остаток // Дебитовый счёт
        {
            ...
            Assert.AreEqual(1220, firstAndreyAccount.AccountStatus); // узнаем какой будет счёт после начисления процента на остаток в октябре 21 года
        ...
        [Test()]
        public void Test3() //Проверим депозитные счета, статус счёта должен быть sleep до момента окончания начисления денег
        {
            DateTime futureDateTime = new DateTime(2021, 10, 2);
            ...
            firstBank.AddDepositAccount(clientAndrey, 300000, DateTime.Now, futureDateTime);//клиент, стартовая сумма(она же определяет процент), дата начала, дата окончания.
            BankAccount firstAndreyAccount = clientAndrey.Accounts.ElementAt(0);
